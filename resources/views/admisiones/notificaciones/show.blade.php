...
<div class="bg-white shadow-md rounded-lg p-6 border border-gray-200 max-w-4xl mx-auto">
    ...
    <div class="mb-6">
        <h2 class="text-2xl font-semibold text-gray-800 mb-2">{{ $notificacion->asunto }}</h2>

        <p class="text-sm text-gray-600 mb-1">
            <strong>Canal de Envío:</strong> {{ ucfirst($notificacion->canal) }}
        </p>

        <p class="text-sm text-gray-600 mb-1">
            <strong>Fecha de Envío:</strong> {{ \Carbon\Carbon::parse($notificacion->fechaEnvio)->format('d/m/Y H:i') }}
        </p>

        <p class="text-sm text-gray-600 mb-1">
            <strong>Destinatarios:</strong> 
            {{ count(explode(',', $notificacion->destinatarios)) }} destinatario(s)
    ...
    <div class="mt-6 flex justify-end">
        <a href="{{ route('notificaciones.por_proceso', $notificacion->procesoID) }}" 
...
<div id="modal-destinatarios" class="hidden fixed inset-0 bg-gray bg-opacity-30 flex justify-center items-center z-50">
    <div class="bg-white p-6 rounded-lg w-full max-w-md relative shadow-lg">
        ...
        <ul class="text-gray-700 text-sm max-h-60 overflow-y-auto">
            @foreach(explode(',', $notificacion->destinatarios) as $destinatario)
