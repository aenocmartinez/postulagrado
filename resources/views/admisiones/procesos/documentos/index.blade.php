...
<div class="bg-white shadow-md rounded-lg p-6 border border-gray-200">
    ...
    <div class="flex justify-between items-center mb-4">
        <h2 class="text-lg font-semibold text-gray-800">
            {{ $procesoDocumento->nombreProceso }}
        </h2>

        <!-- Botón Adjuntar Documento -->
        <a href="{{ route('procesos.documentos.create', $procesoDocumento->procesoID) }}" 
    ...
    <div class="overflow-x-auto">
        <table class="w-full border-collapse rounded-lg overflow-hidden text-sm">
            ...
            <tbody>
                @forelse($procesoDocumento->documentos as $documento)
                    <tr class="border-b border-gray-300 bg-white hover:bg-gray-100 transition">
                        ...
                        <td class="px-4 py-2 text-center flex justify-center gap-4">
                            ...
                            <!-- Botón Eliminar -->
                            <form action="{{ route('procesos.documentos.destroy', [$procesoDocumento->procesoID, $documento['id']]) }}" method="POST" class="inline">
                                @csrf
                                @method('DELETE')
                                <button type="button" 
                                        class="eliminar-btn text-red-600 hover:text-red-800 transition" 
                                        data-url="{{ route('procesos.documentos.destroy', [$procesoDocumento->procesoID, $documento['id']]) }}">
                                    Eliminar
                                </button>
                            </form>
